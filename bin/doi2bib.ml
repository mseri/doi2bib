  ...
  | Some id ->
    ...
    | exception PubMed_DOI_not_found ->
      ...
      @@ Printf.sprintf
           ...
           (string_of_id @@ parse_id id)
    | exception Entry_not_found ->
      err
      @@ Printf.sprintf
           "Error: unable to find any bibtex entry for %s.\nCheck the ID before trying \
            again.\n"
           (string_of_id @@ parse_id id)
    ...
    | exception Bad_gateway ->
      ...
      @@ Printf.sprintf
           "Remote server error: wait some time and try again.\nThis error tends to \
            happen when the remote servers are busy."
    | exception Parse_error id ->
      err
      @@ Printf.sprintf
           "Error: unable to parse ID: '%s'.\nYou can force me to consider it by \
            prepending 'doi:', 'arxiv:' or 'PMC' as appropriate."
           id)
...
let id =
  ...
  let doc =
    "A DOI, an arXiv ID or a PubMed ID. The tool tries to automatically infer what kind \
     of ID you are using. You can force the cli to lookup a DOI by using the form \
...
let doi2bib_t = Cmdliner.Term.(ret (const doi2bib $ id))
