...
module Make
    (C : Cohttp_lwt.S.Client) (Ezgz : sig
      exception GzipError of string

      val gzip_h : Cohttp.Header.t option -> Cohttp.Header.t option
      val extract : bool -> string -> string
    end) =
struct
  ...
  let rec get ?proxy ?headers ?fallback uri =
    ...
    let* resp, body = C.get ?headers uri in
